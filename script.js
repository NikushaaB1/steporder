...
async function init() {
  ...
  try {
    ...
    console.error(e);
    gridEl.innerHTML = `<div class="empty-state">⚠️ Failed to load menu. Please try again.</div>`;
  }
}
...
function bindUI() {
  ...
  spiceSlider.addEventListener('input', (e) => {
    ...
    const pct = (v / 5) * 100;
    spiceSlider.style.background =
      `linear-gradient(to right, #e81c0c ${pct}%, #ddd ${pct}%)`;
  ...
  btnReset.addEventListener('click', () => {
    ...
    spiceSlider.value = 0;
    spiceSlider.style.background = 'linear-gradient(to right, #e81c0c 0%, #ddd 0%)';
...
function renderCards() {
  ...
  if (!list.length) {
    gridEl.innerHTML = `<div class="empty-state">🍽️ No dishes match your filters.</div>`;
    return;
  }
...
function cardHTML(p) {
  const spice = p.spiciness > 0
    ? `<div class="spice-badge">${'🌶️'.repeat(p.spiciness)}</div>`
    : '';
    ...
    <article class="card">
      <div class="card-img">
        ...
        <img src="${p.image}" alt="${p.name}" loading="lazy"
             onerror="this.src='https://via.placeholder.com/400x300/eee/999?text=No+Image'"/>
      </div>
      <div class="card-info">
        <h3 class="card-title">${p.name}</h3>
        <div class="card-tags">
          <span class="${p.nuts ? 'tag-on' : 'tag-off'}">${p.nuts ? '● Nuts' : '○ Nuts'}</span>
          <span class="${p.vegeterian ? 'tag-on' : 'tag-off'}">${p.vegeterian ? '● Vegetarian' : '○ Vegetarian'}</span>
        </div>
...
const PRODUCT_DETAILS = {
  1: {
    desc: "Authentic Thai minced chicken salad with toasted rice powder, fresh herbs, lime, and a hint of chili. A vibrant explosion of flavor in every bite.",
    ing:  ["Minced chicken", "Toasted rice powder", "Lime juice", "Fish sauce", "Mint", "Cilantro", "Shallots", "Chili flakes", "Peanuts"],
  },
  2: {
    desc: "Crisp green papaya tossed with cherry tomatoes, long beans, and a tangy chili-lime dressing. Refreshing, crunchy and 100% vegetarian.",
    ing:  ["Green papaya", "Cherry tomatoes", "Long beans", "Garlic", "Lime juice", "Palm sugar", "Soy sauce", "Chili"],
  },
  3: {
    desc: "Iconic spicy and sour Thai soup with chicken, lemongrass, galangal, and kaffir lime leaves. Bright, fragrant and warming.",
    ing:  ["Chicken", "Lemongrass", "Galangal", "Kaffir lime leaves", "Mushrooms", "Tomato", "Lime juice", "Chili paste"],
  },
  4: {
    desc: "Creamy coconut chicken soup infused with galangal and lime. Mild, silky, and deeply comforting.",
    ing:  ["Chicken", "Coconut milk", "Galangal", "Lemongrass", "Mushrooms", "Lime juice", "Cilantro"],
  },
  5: {
    desc: "Coconut shrimp soup with delicate aromatics and tender prawns. A luxurious bowl of warmth.",
    ing:  ["Shrimp", "Coconut milk", "Galangal", "Lemongrass", "Mushrooms", "Lime", "Fish sauce"],
  },
  6: {
    desc: "Fiery hot & sour shrimp soup — Thailand's most famous bowl. Bold, aromatic, unforgettable.",
    ing:  ["Shrimp", "Lemongrass", "Galangal", "Kaffir lime", "Tomato", "Mushrooms", "Lime juice", "Thai chili"],
  },
  7: {
    desc: "Plant-based version of Tom Yam — bold, sour, spicy and packed with vegetables.",
...
const CATEGORY_FALLBACK = {
  1: {
    desc: "A fresh, vibrant salad bursting with seasonal ingredients and bold dressing.",
    ing:  ["Mixed greens", "Fresh herbs", "Citrus dressing", "Seasonal vegetables", "Spices"],
  },
  2: {
    desc: "A warming bowl of soup, slow-simmered with aromatic herbs and rich broth.",
    ing:  ["Broth", "Aromatic herbs", "Vegetables", "Spices", "Lime juice"],
  },
  3: {
    desc: "Tender, flavorful chicken dish prepared with traditional spices and care.",
    ing:  ["Chicken", "Garlic", "Onion", "Spices", "Herbs", "Sauce"],
  },
  4: {
    desc: "Premium beef dish, slow-cooked for maximum tenderness and flavor.",
    ing:  ["Beef", "Garlic", "Onion", "Spices", "Herbs", "Sauce"],
  },
  5: {
    desc: "Fresh seafood prepared with delicate spices that highlight its natural flavor.",
    ing:  ["Fresh seafood", "Lime", "Herbs", "Garlic", "Spices"],
  },
  6: {
    desc: "A hearty vegetable dish celebrating the best of seasonal produce.",
    ing:  ["Seasonal vegetables", "Herbs", "Garlic", "Olive oil", "Spices"],
  },
  7: {
    desc: "Perfect bite-sized snack, crispy outside and full of flavor inside.",
    ing:  ["Mixed batter", "Herbs", "Spices", "Dipping sauce"],
  },
  8: {
    desc: "The perfect side to complement your main dish.",
    ing:  ["Fresh ingredients", "Herbs", "Light seasoning"],
  },
};

function getProductDetails(p) {
  return PRODUCT_DETAILS[p.id] ||
         CATEGORY_FALLBACK[p.categoryId] || {
  ...
  modalBody.innerHTML = `
    <div class="modal-img">
      ...
      <img src="${p.image}" alt="${p.name}"
           onerror="this.src='https://via.placeholder.com/600x300/eee/999?text=No+Image'"/>
    </div>
    <div class="modal-info">
      ...
      <div class="modal-tags">
        ${p.spiciness > 0
          ? `<span class="modal-tag spice">🌶️ Spice level ${p.spiciness}/5</span>`
          : `<span class="modal-tag">Mild</span>`}
        ${p.vegeterian ? `<span class="modal-tag veg">🌱 Vegetarian</span>` : ''}
        ${p.nuts
          ? `<span class="modal-tag nuts">🥜 Contains nuts</span>`
          : `<span class="modal-tag">Nut-free</span>`}
      </div>
